...
class StockPicking(models.Model):
    ...
    def action_cancel(self):
        if self._sr_get('managers_only_cancel', False) and not self._sr_is_manager():
            for picking in self:
                # A user can still cancel their OWN mistake without needing a
                # manager - the restriction only stops them cancelling
    ...
    def button_validate(self):
        ...
        # Zero-quantity check runs here, before super(), so it fires before
        # any native Odoo zero-quantity check that might otherwise pre-empt it.
