...
class StockPicking(models.Model):
    ...
    # ── ACTION CANCEL — block non-managers cancelling others' in-progress pickings ──

    def action_cancel(self):
        if self._sr_get('managers_only_cancel', False) and not self._sr_is_manager():
            for picking in self:
                # Done/cancelled pickings aren't cancellable via this method in the
                # UI anyway (core Odoo has never had a Cancel button on a Done
                # picking, in any version - Return is the intended undo path), so
                # this targets in-progress operations (draft/waiting/confirmed/
                # assigned). A user can still cancel their OWN mistake without
                # needing a manager - the restriction only stops them cancelling
                # operations created by someone else.
                if picking.state not in ('done', 'cancel') and picking.create_uid.id != self.env.user.id:
                    restriction = _('Manager Only: Cancel Operation')
                    details = _('User "%s" tried to cancel operation "%s" created by "%s".') % (
                        self.env.user.name, picking.name, picking.create_uid.name
                    )
                    self._sr_log(restriction, details, picking=picking)
                    raise UserError(_(
                        'Only Inventory Managers can cancel operations created by other users.\n\n'
                        'Operation: %s\n'
                        'Created by: %s\n\n'
                        'Please contact your Inventory Manager, or cancel operations you created yourself.'
                    ) % (picking.name, picking.create_uid.name))
    ...
    def button_validate(self):
        ...
        # Zero-quantity check must run here, BEFORE Odoo core's own
        # button_validate logic, since core raises its own zero-quantity
        # message before _action_done is ever reached.
        if self._sr_get('block_zero_qty', False) and not self._sr_is_admin():
            for picking in self:
    ...
    def _action_done(self):
        ...
        for picking in self:
            for move in picking.move_ids.filtered(lambda m: m.state not in ('done', 'cancel')):
                ...
                # Get done quantity. Odoo 17+ renamed stock.move.line.qty_done to
                # 'quantity' and removed stock.move.quantity_done entirely in favor
                # of a direct 'quantity' field on stock.move itself.
                total_done = move.quantity
