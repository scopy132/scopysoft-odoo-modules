...
class VendorPriceHistory(models.Model):
    ...
    def _notify_if_significant(self):
        """Post an activity to the purchase responsible / product responsible
        when a price change crosses the configured alert threshold."""
        for rec in self:
            ...
            user = rec.product_tmpl_id.responsible_id if 'responsible_id' in rec.product_tmpl_id._fields else False
            note_user_id = user.id if user else rec.env.user.id
            try:
                rec.product_tmpl_id.activity_schedule(
